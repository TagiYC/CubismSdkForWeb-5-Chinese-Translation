...
/**
 * 不使用CubismJson的Json解析器，
 * 而是使用TypeScript标准Json解析器，
 * 并将其输出结果替换为Cubism SDK定义的JSON元素。
 */
  ...
  static parseJsonObject(obj: Value, map: JsonMap) {
    Object.keys(obj).forEach(key => {
      if (typeof obj[key] == 'boolean') {
        ...
        // HACK: Array 单体无法转换，因此将其转换为 unknown 并转换为 Value
        map.put(
          key,
    ...
    Object.keys(obj).forEach(key => {
      ...
      if (typeof convKey == 'number') {
        if (typeof obj[key] == 'boolean') {
          ...
          // HACK: Array不能单独转换，所以先用unknown再用Value。
